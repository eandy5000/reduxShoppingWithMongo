...
function deleteBook(store, _id) {
    _axios2.default.delete('/books/' + _id).then(function (res) {
        ...
        return console.log('action err ', err);
    });
}

// function needs store, updated item (with title and price) and _id
function updateBook(store, _id, update) {
    _axios2.default.put('/books/' + _id, update).then(function (res) {
        store.dispatch({
            type: 'UPDATE_BOOK',
            payload: res.data
        });
...
var _countReducer = __webpack_require__(39);

var _booksReducer = __webpack_require__(38);
...
var _countActions = __webpack_require__(17);

var _booksActions = __webpack_require__(16);
...
(0, _booksActions.updateBook)(store, '596bfb0b8497598921edd5a1', { title: 'ocho', price: 3 });
...
function booksReducer() {
    ...
    switch (action.type) {
        ...
        case 'DELETE_BOOK':
            ...
            return {
                books: [].concat(_toConsumableArray(state.books.slice(0, delInd)), _toConsumableArray(state.books.slice(delInd + 1))),
                error: ''
            };
        ...
        case 'UPDATE_BOOK':
            ...
            var upInd = state.books.findIndex(function (book) {
                return book._id === action.payload._id;
            });
            return {
                books: [].concat(_toConsumableArray(state.books.slice(0, upInd)), [action.payload], _toConsumableArray(state.books.slice(upInd + 1))),
                error: ''
            };
